...
<div class="container mt-5">
    ...
    <div id="lineupForm" class="mt-4" style="display: none;">
        <form action="{{ route('lineup.store') }}" method="POST" class="shadow p-4 rounded border">
            @csrf
    ...
    @if($lineup->team_id == $team->id) <!-- Check if the lineup belongs to the current team -->
        <div class="card mb-3 shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span>Lineup for: {{ $lineup->match->homeTeam->name }} vs {{ $lineup->match->awayTeam->name }}</span>
                ...
                <div class="lineup-details d-none">
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6">
                            <h6>Substitutes:</h6>
                            <ul>
                                @foreach($lineup->players->where('pivot.position_type', 'Substitute') as $player)
                                    <li>{{ $player->name }} - {{ $player->position }}</li>
                                @endforeach
                            </ul>
                        </div>
...
<script>
    ...
    document.querySelector("form[action='{{ route('lineup.store') }}']").addEventListener("submit", function (e) {
