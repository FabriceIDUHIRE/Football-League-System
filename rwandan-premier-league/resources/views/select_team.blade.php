...
<body class="min-h-screen flex flex-col justify-center items-center text-white">
    ...
    <div class="container mx-auto flex flex-wrap justify-center gap-8">
        ...
        <div class="glass-card p-8 text-center w-80 transform transition duration-500 hover:scale-105">
            <img src="{{ asset('storage/' . $team->logo) }}" alt="{{ $team->name }}" class="w-32 h-32 mx-auto rounded-full border-4 border-white shadow-lg">
            <h2 class="text-2xl font-bold mt-4 text-gray-900">{{ $team->name }}</h2>
            <p class="text-gray-700 italic mb-4">{{ $team->city }}</p>
            
            <!-- Updated Link to Show Team Details -->
            <a href="{{ route('team.show', ['teamId' => $team->id]) }}" class="text-blue-600 font-bold text-lg hover:text-blue-800">Select Your Team</a>
        </div>
        @endforeach
    </div>
