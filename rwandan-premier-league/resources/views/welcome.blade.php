...
<nav class="bg-white text-black py-4 px-6 shadow-md">
    <div class="container mx-auto flex items-center justify-between">
        <!-- Team Logo -->
        <img src="{{ asset('storage/' . $team->logo) }}" alt="Team Logo" class="h-16 w-auto">
        
        <!-- Desktop Menu -->
        <div class="hidden lg:flex space-x-6 text-lg font-medium">
        <a href="{{ route ('select.team') }}" class="hover:text-gray-500">Home</a>
...
<!-- Team Header -->
<header class="relative w-full h-80 bg-cover bg-center flex items-center justify-center" style="background-image: url('{{ $team->logo }}');">
    <div class="text-center text-white">
        <img src="{{ asset('storage/' . $team->logo) }}" alt="{{ $team->name }}" class="mx-auto h-24 w-auto mb-4">
        <h1 class="text-4xl font-bold">{{ $team->name }}</h1>
        <p class="text-lg">{{ $team->location }}</p>
    </div>
</header>
    ...
    <div class="container mx-auto text-center">
        ...
        <div class="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="col-span-1 lg:col-span-2">
                ...
                <img src="{{ asset('storage/' . $team->posts->first()->image) }}" 
     alt="{{ $team->posts->first()->title }}" 
     class="w-full h-[500px] object-cover rounded-lg">

                    <h3 class="text-4xl font-bold mt-6 text-blue-800">{{ $team->posts->first()->title }}</h3>
                    <p class="mt-4 text-lg text-gray-700">{{ \Illuminate\Support\Str::limit($team->posts->first()->content, 300) }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                @foreach($team->posts->skip(1) as $post)
                <div class="bg-white shadow-md p-5 rounded-md">
                <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="w-full h-48 object-cover rounded-md">
            ...
            @else
            <p class="text-2xl text-gray-600">No news posted yet for {{ $team->name }}</p>
            @endif
        </div>
    </div>
</section>
...
<section class="team-history py-10 bg-gray-50" id="team-history">
    ...
    <p class="text-lg">{{ $team->history }}</p>
    </div>
</section>
...
<section class="team-roster py-10 bg-gray-50" id="team-roster">
    <h2 class="text-3xl font-bold text-center text-gray-800">Team Roster - {{ $team->name }}</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-6 container mx-auto">
        @foreach($players as $player)
        <div class="player-card bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
        <img src="{{ asset('storage/' . $player->image) }}" alt="{{ $player->name }}" class="w-32 h-32 object-cover mx-auto rounded-full">
        <h3 class="text-xl font-semibold text-center mt-4 text-gray-900">{{ $player->name }}</h3>
            <p class="text-center text-gray-600">{{ $player->position }}</p>
            <p class="text-center text-gray-500 mt-2">Country: {{ $player->nationality }}</p>
            <p class="text-center text-gray-500">Jersey Number#: {{ $player->jersey_number }}</p>
        </div>
        @endforeach
    </div>
</section>
            ...
            <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6 border border-gray-200">
                <div class="text-center">
                    ...
                    <p class="text-lg text-gray-700 mt-4">
                        📅 
                        <span class="font-medium">{{ \Carbon\Carbon::parse($match->match_date)->format('l, F j, Y') }}</span>
                    </p>

                    <p class="text-lg text-gray-700">
                        🏟️ 
                        <span class="font-medium">{{ $match->stadium->name ?? 'Unknown Stadium' }}</span>
                    </p>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-12 mt-8 container mx-auto px-6">
        @forelse($sponsors as $sponsor)
            <div class="sponsor-card bg-white flex items-center justify-center h-52 w-52 text-center rounded-full border-4 border-gray-400 shadow-lg">
                <h3 class="text-2xl font-bold text-gray-700">{{ $sponsor->sponsor_name }}</h3>
            </div>
        @empty
...
<section class="fan-feedback py-10 bg-gray-100" id="fan-feedback">
    ...
    <form action="{{ route('feedback.store', ['team_id' => $team->id]) }}" method="POST">
    @csrf
